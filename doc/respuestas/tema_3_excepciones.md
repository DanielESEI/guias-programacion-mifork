...
**IOException** (AccesDeniedException) - (Ficheros)
...
try {
    ...
    // Se ejecuta siempre que se entre en el bloque try
    if (file != null) {
...
- Controlada: `IOException` (archivo no encontrado), `FileNotFoundException`. Se obliga a `try-catch / throw`.
- No controlada: `NullPointerException`("NPE"), `ArrayIndexOutOfBoundsException`(posiciones fueras del array), `IllegalArgumentException`. No obliga a `try-catch / throw`.
...
- Lectura/escritura de ficheros (`IOException`) - Entrada/Salida
...
public void leerArchivo(String ruta) throws IOException {
    BufferedReader br = new BufferedReader(new FileReader(ruta)); // = Files.readAllBytes(p); --- con try/catch o throws
...
// try {
    ...
    thorw new RuntimeException("Excepción de e/s");
}
```
...
// try {
    ...
    thorw new AplicaccionException("error");
}
```
...
- Se ve cuando la excepción se muestra por pantalla.
        ...
        Excepcion externa (NextfuxException)

        -----

        Caused by excepcion interna (IOException)

- Se puede obtener en el método "getCause()"
